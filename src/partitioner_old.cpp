...
Partitioner_old::Partitioner_old(ModelImpl* model, CompilerOptions::GraphPartitioningScheme gp)
    : model_(model), gp_(gp)
{
    switch(gp_) {
...
bool Partitioner_old::isVMVMUAssigned(Operation* op) {
    return op2vmvmu_.count(op);
}

void Partitioner_old::assignVMVMU(Operation* op, unsigned int vMVMU) {
    assert((!isVMVMUAssigned(op)) && "Cannot reassign virtual MVMU!");
    op2vmvmu_[op] = vMVMU;
}

void Partitioner_old::cloneAssignment(Operation* cloneFrom, Operation* cloneTo) {
    if(isVMVMUAssigned(cloneFrom)) {
        assignVMVMU(cloneTo, getVMVMU(cloneFrom));
    }
}

unsigned int Partitioner_old::getVMVMU(ConstantMatrixTile* tile) {
    assert(cmat2vmvmu_.count(tile) && "Virtual MVMU not assigned!");
    return cmat2vmvmu_[tile];
}

unsigned int Partitioner_old::getVCore(ConstantMatrixTile* tile) {
    return vmvmu2vcore_[getVMVMU(tile)];
}

unsigned int Partitioner_old::getVTile(ConstantMatrixTile* tile) {
    return vcore2vtile_[getVCore(tile)];
}

unsigned int Partitioner_old::getVMVMU(TrainingMatrixTile* tile) {
    assert(tmat2vmvmu_.count(tile) && "Virtual MVMU not assigned!");
    return tmat2vmvmu_[tile];
}

unsigned int Partitioner_old::getVCore(TrainingMatrixTile* tile) {
    return vmvmu2vcore_[getVMVMU(tile)];
}

unsigned int Partitioner_old::getVTile(TrainingMatrixTile* tile) {
    return vcore2vtile_[getVCore(tile)];
}

unsigned int Partitioner_old::getVMVMU(Operation* op) {
    assert(isVMVMUAssigned(op) && "Virtual MVMU not assigned!");
    return op2vmvmu_[op];
}

unsigned int Partitioner_old::getVCore(Operation* op) {
    return vmvmu2vcore_[getVMVMU(op)];
}

unsigned int Partitioner_old::getVTile(Operation* op) {
    return vcore2vtile_[getVCore(op)];
}

void Partitioner_old::assignVMVMUsInRowMajor() {
...
void Partitioner_old::assignVMVMUsInColMajor() {
...
void Partitioner_old::assignVMVMUsRandomly() {
...
void Partitioner_old::assignVMVMUsAndSpreadAffinity() {
...
void Partitioner_old::spreadVMVMUAffinityToOperands(ConsumerOperation* op) {
...
void Partitioner_old::spreadVMVMUAffinityToUsers(ProducerOperation* op) {
...
void Partitioner_old::assignVCoresInVMVMUOrder() {
...
void Partitioner_old::assignVTilesInVMVMUOrder() {
    ...
    vtileType_[0] = vtileType_[1] = 0;
    for(unsigned int vCore = 2; vCore < nVCores_; ++vCore) {
        unsigned int vTile = (vCore - 2)/N_CORES_PER_TILE + 2;
        vcore2vtile_[vCore] = vTile;
        vtileType_[vTile] = vcoreType_[vCore];
    }
...
void Partitioner_old::assignVCoresWithKaHIP() {
...
void Partitioner_old::assignVTilesWithKaHIP() {
...
void Partitioner_old::assignVCoresWithStorageType() {
...
void Partitioner_old::assignVTilesWithStorageType() {
...
void Partitioner_old::insertLoadsAndStores() {
...
void Partitioner_old::insertSendsAndRecives() {
...
void Partitioner_old::insertInputAndOutput() {
...
void Partitioner_old::insertCopies() {
...
void Partitioner_old::unlink(Operation* op) {
    op2vmvmu_.erase(op);
    model_->unlink(op);
}

std::string Partitioner_old::printAssignment(Operation* op) {
    std::stringstream ss;
...
void Partitioner_old::printReport(std::ofstream& report) {
    switch(gp_) {
        case CompilerOptions::GP_ROW_MAJOR:
