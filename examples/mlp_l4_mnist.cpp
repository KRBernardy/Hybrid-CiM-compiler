...
int main() {
    ...
    Model model = Model::create("mlp_l4_mnist");
    ...
    // Define network
    auto out1 = linear(model, "layer" + std::to_string(1), in_size1, out_size1, in);
    auto out1_activate = relu(out1);
    auto out2 = linear(model, "layer" + std::to_string(2), in_size2, out_size2, out1_activate);
    auto out2_activate = relu(out2);
    auto out3 = linear(model, "layer" + std::to_string(3), in_size3, out_size3, out2_activate);
    auto out3_activate = relu(out3);
    auto out4 = linear(model, "layer" + std::to_string(4), in_size4, out_size4, out3_activate);
    out = out4;
    ...
    modelInstance.load("layer" + std::to_string(1), layer1Weights);
    modelInstance.load("layer" + std::to_string(2), layer2Weights);
    modelInstance.load("layer" + std::to_string(3), layer3Weights);
    modelInstance.load("layer" + std::to_string(4), layer4Weights);
